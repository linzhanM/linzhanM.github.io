...
This is a personal academic homepage for Linzhan Mou, deployed as a GitHub Pages static site at the custom domain `linzhanmou.com` (set by the root `CNAME`; `linzhanm.github.io` 301-redirects to it). All absolute self-links — in `index.html`, `resume/`, `dimo/`, `unimate/`, `sitemap.xml`, `robots.txt`, and `resume/latex/resume.tex` — must use `linzhanmou.com`. It is a single-page site with no build system, no package manager, and no server-side code.
...
**`unimate/`**: Self-contained project page for the UniMate paper ("One Unified Model to Animate Diverse Skeletons"), now public and indexable — `index, follow` plus a canonical URL and a real `description`, matching `dimo/`, and listed in `sitemap.xml`. No Bootstrap — all styling is in `unimate/style.css`; markup is in `unimate/index.html`. The interactive viewer is **no longer inline**: it is two external ES modules loaded via an importmap — `unimate/js/viewer.js` (the three@0.160.0 rendering engine) imports `EXAMPLES` from `unimate/js/examples.js` (a data-only scene catalog). `style.css` `@import`s Playfair Display (headings) + Roboto Mono (labels/mono) from Google Fonts, over a Georgia serif body.
