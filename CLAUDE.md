...
**Prompts are content**, so they live in `unimate/resources/prompts.json`, keyed by glb filename; `examples.js` carries a `prompt` only to override (`''` suppresses). House style is the paper's own captioning format (Appendix A.5): `'An [object type] [action].'` — one sentence, under 12 words, one dominant action, no adverbs, the object named concretely. **Never invent a prompt from a filename.** Two ways to find out what a clip does, and `prompts.json` records which was used (`_from_caption` = the original): 22 rigs are captioned in `assets/videos/diverse-1..3.mp4` and the `prompt-*` clips (pull a frame with ffmpeg — the posters are t=0 and blank); for the rest, render the motion headlessly in Blender. That pass corrected real guesses (`ironman-walk` and the quadrupeds walk **in place**, `whale-sweep` never travels, `dragon_fire-hover` hovers). A render shows the motion but **not the dataset's word for it**, which is the failure mode that actually bit — reach for a verb already attested in `_from_caption` before coining a precise one.
