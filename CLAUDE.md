...
- **JavaScript** is split: the viewer is two external ES modules loaded via an importmap — `unimate/js/viewer.js` (the three@0.160.0 engine, from jsDelivr) imports `EXAMPLES` from `unimate/js/examples.js` (data only). But **two inline `<script>` blocks remain** at the end of `index.html`, and they are where the page's own behaviour lives. Each holds two named jobs in one `DOMContentLoaded`. The first is the galleries: the horizontal-scroll buttons, which navigate by *live* item position rather than a cached width because a video's intrinsic size isn't known that early, and an `IntersectionObserver` (`threshold: 0.15`) that plays only on-screen videos and switches every video to `controls` under `prefers-reduced-motion`. The second is the TOC rail: `scrollSpy()`, and `collisionWatcher()` — the galleries are full-bleed (`100vw`) while the rail is fixed in the left gutter, so a wide strip runs under it, and the watcher fades the rail out (`.toc.is-eclipsed`) while that lasts. It measures the **clips**, not `.video-gallery-container`: that box is a fixed 1150px centred frame, so on the single-clip Applications galleries it reaches the gutter while the clip itself is nowhere near it. Then StatCounter (project `12925377`, shared with the homepage).
