...
**The rigs in `resources/glbs/` are compressed** (meshopt geometry + 512px WebP, 671 MB → 50 MB), so `viewer.js` registers `MeshoptDecoder` on its `GLTFLoader` — **drop that and every rig fails to parse**. WebP and quantization are native to three.js; meshopt is not. Uncompressed sources sit in the gitignored `resources/glbs-raw/` (the repo is the deployed artifact, and Pages caps a site at 1 GB). That folder is now the **only** copy: history was purged on 2026-08-06 (the old `b4ac4c8` recovery path is gone) because 2.3 GB of unreachable blobs had grown the repo to 1.63 GB and were stalling the Pages build past its 15-minute timeout. Keep history lean — a large file committed and then deleted still costs forever. A new rig must be run through the pipeline in `unimate/README.md` before it is committed.
