...
// Both pages share the catalog; only the viewer config differs. Stages are
// hidden here by label — viewer-presets.js says what that costs.
...
// with WebP textures — 736 MB down to 59 MB; pipeline in unimate/README.md. WebP
// is native to GLTFLoader, but meshopt geometry needs this decoder registered or
// every .glb fails to parse.
...
// materials (the mesh vanishes); a black diffuse multiplied with an embedded
// texture renders flat; and external textures (a sibling .fbm folder) aren't
// shipped, so those maps never get image data. Repair opacity now and tag maps
// as sRGB; the map/colour decisions wait on actual load state below.
function fixMaterials(model, isFbx) {
  ...
  // Resolve each map by its actual load state, not a guessed delay that could
  // drop a slow-decoding embedded texture. Loaded: keep it (and lift the parked
  // black to white). Definitively failed — a missing external .fbm texture — or
  // still pending after 8 s: drop it, so the surface shows a clean base colour
  // instead of an empty sample.
  if (mapped.length) {
    ...
    const tick = () => {
      ...
      for (const m of mapped) {
        ...
        if (loaded) {
          if (m.__liftWhite) m.color.setRGB(1, 1, 1);
...
// ── 7. Stage: framing + teardown ─────────────────────────────────────────────
// Shared checkerboard floor texture. Each theme paints its own two tones;
// repeat is set per rebuild to size the squares.
...
// Frame the whole stage: fit the camera to the union of all pivots, drop a checker
...
function frameStage(pad = 1.0, orbitAngleDegrees = viewerConfig.initialOrbitAngle || 0) {
  ...
  // (Re)build the checker floor sized to the stage — at least the framed width, so
  ...
  const squares = 24;                          // across the whole floor
  ...
  // falls, so the checker shows through elsewhere.
...
// The toolbar is opaque and above the label layer, so a chip under it loses its
// text. Measure it in wrapper coordinates; updateLabels treats it as an obstacle
// chips slide left of instead of vanishing behind.
let guiBox = null;
function measureGui() {
...
// Controls take the same reduction as the chips, with a readability floor. The
// embed starts from a 160px desktop panel, the lab from 210px; separate floors
// land both at a comparable density on a phone instead of an embed gone
// microscopic or a lab panel dominating the stage.
...
// The toolbar is an obstacle in the slot scoring too, and a costly one. Otherwise
// a chip whose model sits under the panel still picks "above", collides, and is
// shoved sideways by the dodge — a long diagonal leader away from its rig. Scored,
// it simply goes below its model, where it is near.
const GUI_COST = 1.5;
...
function updateLabels(dt) {
  ...
  for (let i = 0; i < visible.length; i++) {
    ...
    for (let pass = 0; pass < visible.length; pass++) {
      // The toolbar can't be lifted past, so slide clear to its left (measureGui).
      // Inside the loop because a lift can carry a chip back up into its band.
...
function loadExample(index) {
  ...
  // stage-tuning.js may override this stage's layout options for the current
  // page; the files never differ.
...
// ── 11. UI wiring ────────────────────────────────────────────────────────────
// Toolbar (lil-gui), pinned inside the viewer. Flat — no nested folders.
...
// No Wireframe row: lil-gui shows only on the embedded page, which dropped the
// option; the lab hides lil-gui and carries wireframe in its own dock.
...
// The lab drives the same settings from a keyboard-first dock. lil-gui stays the
// embedded viewer's control surface and is hidden on interactive.html, so the
// stage has one set of controls.
...
if (isFullscreenLab) {
  ...
  // the opening pose; playing, the motion starts over. lil-gui's "Reset view"
  // stays camera-only, as its name promises.
  const resetStage = () => {
...
// On phones, scale the whole Controls panel from its pinned top-right corner
// rather than restyling rows. measureGui's getBoundingClientRect() reports the
// transformed size, so chips avoid the panel's visible footprint.
function applyResponsiveControlScale() {
  ...
  // Scale the inset with the panel so its alignment reads the same; max() keeps
  // it clear of notches in fullscreen.
...
// Collapsing the panel frees the space under it; the chips' no-go box must follow.
